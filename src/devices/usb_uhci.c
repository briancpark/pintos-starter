...
static int uhci_tx_pkt(host_eop_info eop, int token, void* pkt, int min_sz, int max_sz, int* in_sz,
                       bool wait);
...
#define uhci_is_stopped(x) (pci_reg_read16((x)->io, UHCI_REG_USBSTS) & USB_STATUS_HALTED)
...
static void uhci_setup_td(struct tx_descriptor* td, int dev_addr, int token, int eop, void* pkt,
                          int sz, int toggle, bool ls);
...
static struct usb_host uhci_host = {.name = "UHCI",
                                    .tx_pkt = uhci_tx_pkt,
...
void uhci_init(void) {
  ...
  dev_num = 0;
  while ((pd = pci_get_dev_by_class(PCI_MAJOR_SERIALBUS, PCI_MINOR_USB, PCI_USB_IFACE_UHCI,
                                    dev_num)) != NULL) {
    struct pci_io* io;
    struct uhci_info* ui;
    int i;
    ...
    pci_reg_write16(ui->io, UHCI_REG_USBCMD, USB_CMD_RS | USB_CMD_CONFIGURE | USB_CMD_MAX_PACKET);
    pci_reg_write16(ui->io, UHCI_REG_USBINTR,
                    USB_INTR_SHORT | USB_INTR_IOC | USB_INTR_TIMEOUT | USB_INTR_RESUME);
...
static int uhci_enable_port(struct uhci_info* ui, int idx) {
  uint16_t status;
  ...
  for (time = 0;; time += 25) {
    ...
    if (status != (new_status & USB_PORT_CONNECTSTATUS) || new_status & USB_PORT_CONNECTCHG) {
...
static void dump_regs(struct uhci_info* ui) {
  ...
  char* name[] = {"cmd", "sts", "intr", "frnum", "base", "sofmod", "portsc1", "portsc2"};
  int i;
  printf("UHCI registers:\n");
  for (i = 0; i < 8; i++) {
    printf("%s: %x\n", name[i],
           (sz[i] == 2) ? pci_reg_read16(ui->io, regs[i]) : pci_reg_read32(ui->io, regs[i]));
  }
}
...
static int uhci_tx_pkt(host_eop_info hei, int token, void* pkt, int min_sz, int max_sz, int* in_sz,
                       bool wait) {
...
static int uhci_tx_pkt_bulk(struct uhci_eop_info* ue, int token, void* buf, int sz, int* tx) {
  ...
  while (bytes < sz) {
    ...
    pkt_txed = 0;
    err = uhci_tx_pkt(ue, token, buf + bytes, 0, to_tx, &pkt_txed, wait_on_pkt);
    if (err) {
      if (tx != NULL)
        *tx = txed;
      return err;
...
static void uhci_setup_td(struct tx_descriptor* td, int dev_addr, int token, int eop, void* pkt,
                          int sz, int toggle, bool ls UNUSED) {
...
static int uhci_tx_pkt_now(struct uhci_eop_info* ue, int token, void* pkt, int sz) {
  ...
  uhci_setup_td(td, ud->dev_addr, token, ue->eop, pkt, sz, ue->toggle, ud->low_speed);
...
static int uhci_tx_pkt_wait(struct uhci_eop_info* ue, int token, void* pkt, int max_sz,
                            int* in_sz) {
  ...
  uhci_setup_td(td, ud->dev_addr, token, ue->eop, pkt, max_sz, ue->toggle, ud->low_speed);
  td->control.ioc = 1;
...
static int uhci_process_completed(struct uhci_info* ui) {
  ...
  while (start < TD_ENTRIES) {
    ...
    if (!td->control.active && (td->flags & TD_FL_ASYNC) && (td->flags & TD_FL_USED)) {
...
static int uhci_detect_change(host_info hi) {
  struct uhci_info* ui;
  int change;
  int i;
...
static int check_and_flip_change(struct uhci_info* ui, int port) {
  int val;
  int reg;
...
static void uhci_destroy_chan(host_dev_info hd) {
  struct uhci_dev_info* ud;
  struct list_elem* li;
...
static struct tx_descriptor* uhci_acquire_td(struct uhci_info* ui) {
  size_t td_idx;
  struct tx_descriptor* td;
...
static void uhci_modify_chan(host_dev_info hd, int dev_addr, int ver) {
  struct uhci_dev_info* ud;
...
static void uhci_set_toggle(host_eop_info he, int toggle) {
  struct uhci_eop_info* ue;
...
static int uhci_remove_stalled(struct uhci_info* ui) {
  ...
  while (li != list_end(&ui->waiting)) {
    ...
    if ((!uw->td->control.active && uw->td->control.stalled) || (uw->td->control.nak)) {
...
static void uhci_release_td(struct uhci_info* ui, struct tx_descriptor* td) {
...
static void uhci_remove_eop(host_eop_info hei) { free(hei); }
