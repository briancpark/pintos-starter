...
static struct usb_dev* usb_configure_default(struct host* h) {
  ...
  err = h->dev->tx_pkt(cfg_eop, USB_TOKEN_SETUP, &sp, 0, sizeof(sp), NULL, true);
  ...
  if (dd->max_pktsz != 8 && dd->max_pktsz != 16 && dd->max_pktsz != 32 && dd->max_pktsz != 64)
    goto error;
  ...
  while (txed < sizeof *dd) {
    err = h->dev->tx_pkt(cfg_eop, USB_TOKEN_IN, data + txed, 0, sizeof *dd - txed, &sz, true);
    if (err)
      goto error;
    txed += sz;
  }
  ...
  if (dd->num_configs > 0 && ignore_device == false) {
    ...
    if (config_val < 0) {
      printf("USB: Invalid configuration value %d on '%s (%s)' (%x,%x,%x)\n", config_val,
             dev->product, dev->manufacturer, dev->vendor_id, dev->product_id, dev->device_id);
    }
  }
...
static int usb_load_config(struct usb_dev* dev, int idx, void* data, int dsz) {
  ...
  sz = usb_tx_all(&dev->cfg_eop, cd, dsz, sizeof(struct config_descriptor), true);
  ...
  for (i = 0; i < cd->num_ifaces; i++) {
    ...
    if (iface->hdr.type != SETUP_DESC_IFACE) {
      ...
      PANIC("Expected %d, found %d\n", SETUP_DESC_IFACE, iface->hdr.type);
    }
    ...
    for (j = 0; j < iface->num_endpoints; j++, ptr += sizeof(struct endpoint_descriptor)) {
...
static char* usb_get_string(struct usb_dev* udev, int ndx) {
  struct usb_setup_pkt sp;
  ...
  if (udev->host->dev->tx_pkt(udev->h_cfg_eop, USB_TOKEN_SETUP, &sp, 0, sizeof(sp), NULL, true) !=
      USB_HOST_ERR_NONE)
    return NULL;

  sz = usb_tx_all(&udev->cfg_eop, &str, MAX_USB_STR, 2, true);
  sz += usb_tx_all(&udev->cfg_eop, str + sz, (uint8_t)(str[0]) - sz, 0, true);
...
int usb_dev_setup(struct usb_endpoint* eop, bool in, struct usb_setup_pkt* s, void* buf, int sz) {
  ...
  err = h->dev->tx_pkt(eop->h_eop, USB_TOKEN_SETUP, s, 0, sizeof(struct usb_setup_pkt), NULL, true);
  if (err != USB_HOST_ERR_NONE) {
    printf("usb_dev_setup: failed\n");
    return 0;
  }

  err = h->dev->tx_pkt(eop->h_eop, (in) ? USB_TOKEN_IN : USB_TOKEN_OUT, buf, 0, sz, &sz, true);
...
static int usb_tx_all(struct usb_endpoint* eop, void* buf, int max_bytes, int bailout, bool in) {
  ...
  while (txed < max_bytes && txed < bailout) {
    int sz, err;
    sz = 0;
    err = h->dev->tx_pkt(eop->h_eop, token, buf + txed, 0, max_bytes - txed, &sz, true);
    if (prev_sz == 0)
      prev_sz = sz;
    txed += sz;
