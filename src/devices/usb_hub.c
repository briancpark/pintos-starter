...
static struct usb_class hub_class = {
    .attached = hub_attached, .detached = hub_detached, .class_id = USB_CLASS_HUB, .name = "hub"};

void usb_hub_init(void) { usb_register_class(&hub_class); }

static void* hub_attached(struct usb_iface* ui UNUSED) { return NULL; }
