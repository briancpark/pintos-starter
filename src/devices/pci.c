...
#define pci_config_offset(bus, dev, func, reg)                                                     \
...
struct pci_io* pci_io_enum(struct pci_dev* pio, struct pci_io* last) {
  struct list_elem* e;
...
void pci_write_out(struct pci_io* pio UNUSED, int off UNUSED, size_t size UNUSED,
...
static void pci_setup_io(struct pci_dev* pd) {
  ...
  for (i = 0; i < PCI_BASE_COUNT; i++) {
    ...
    pci_write_config(pd->bus, pd->dev, pd->func,
                     offsetof(struct pci_config_header, pci_base_reg[i]), 4, ~0);
    ...
    if (tmp & PCI_BASEADDR_IO) {
      ...
      pio->addr.ptr = pci_alloc_mem((void*)pd->pch.pci_base_reg[i], pio->size / PGSIZE);
      if (pio->addr.ptr == NULL) {
        printf("PCI: %d pages for %d:%d.%d failed - may not work\n", pio->size / PGSIZE, pd->bus,
               pd->dev, pd->func);
        free(pio);
        pio = NULL;
      } else {
...
static void pci_power_on(struct pci_dev* pd UNUSED) { /* STUB */
}
...
static int pci_pci_bridge(struct pci_dev* pd) {
  ...
  command &= ~0x3;
  pci_write_config(pd->bus, pd->dev, pd->func, offsetof(struct pci_config_header, pci_command), 2,
                   command);
...
static void pci_interrupt(struct intr_frame* frame) {
  struct list_elem* e;
  int int_line;
...
static void pci_print_dev_info(struct pci_dev* pd) {
  printf("PCI Device %d:%d.%d (%x,%x): %s - %s (%s) IRQ %d\n", pd->bus, pd->dev, pd->func,
         pd->pch.pci_vendor_id, pd->pch.pci_device_id, pci_lookup_vendor(pd->pch.pci_vendor_id),
         pci_lookup_device(pd->pch.pci_vendor_id, pd->pch.pci_device_id),
         pci_lookup_class(pd->pch.pci_major, pd->pch.pci_minor, pd->pch.pci_interface),
         pd->pch.pci_int_line);
}

void pci_mask_irq(struct pci_dev* pd) { intr_irq_mask(pd->pch.pci_int_line); }

void pci_unmask_irq(struct pci_dev* pd) { intr_irq_unmask(pd->pch.pci_int_line); }
