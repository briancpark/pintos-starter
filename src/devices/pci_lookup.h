...
struct pci_vendor pci_vendor_table[] = {{0x1013, "Cirrus Logic"},
                                        {0x10de, "nVidia"},
                                        {0x10EC, "Realtek"},
...
struct pci_device pci_device_table[] = {
    {0x1013, 0x00b8, "CL-GD5446"},   {0x10ec, 0x8029, "RTL8029 - NE2K Compatible"},
    {0x10ec, 0x8139, "RTL8139"},     {0x8086, 0x1237, "82441FX"},
    {0x8086, 0x7000, "82371SB_ISA"}, {0x8086, 0x7010, "82371SB_IDE"},
    {0x8086, 0x7020, "82371SB_USB"}, {0x8086, 0x7113, "82371AB/EB/MB_ACPI"}};
...
const char* pci_lookup_device(uint16_t vendor, uint16_t device) {
  ...
  for (i = 0; i < PCI_DEVICE_TABLE_SZ; i++) {
    if (pci_device_table[i].vendor_id > vendor)
      break;
    if (pci_device_table[i].vendor_id == vendor && pci_device_table[i].device_id == device)
...
struct pci_class pci_class_table[] = {{0, 0, 0, "Pre-PCI 2.0 Non-VGA Device"},
