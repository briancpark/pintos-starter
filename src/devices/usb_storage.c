...
static struct usb_class storage_class = {.attached = msc_attached,
...
static class_info msc_attached(struct usb_iface* ui) {
  static int dev_no;
  ...
  mci->bounce_buffer = palloc_get_multiple(PAL_ASSERT, DIV_ROUND_UP(mci->blk_size, PGSIZE));
  mbi = malloc(sizeof(struct msc_blk_info));
  mbi->mci = mci;
  snprintf(name, sizeof name, "ud%c", 'a' + dev_no++);
  block = block_register(name, BLOCK_RAW, "USB", mci->blk_count, &msc_operations, mbi);
  partition_scan(block);

  return mci;
}

static void msc_detached(class_info ci UNUSED) { PANIC("msc_detached: STUB"); }
...
static void msc_get_geometry(struct msc_class_info* mci) {
  struct msc_cbw cbw;
...
static void msc_io(struct msc_class_info* mci, block_sector_t bn, void* buf, bool wr) {
  struct msc_cbw cbw;
  struct msc_csw csw;
  struct scsi_cdb10* cdb;
  int tx;
  ...
  *((uint32_t*)((uint8_t*)(&cdb->lba) + 1)) = machine_to_be24(bn);
  ...
  /* do storage io */
  err = usb_dev_bulk((wr) ? mci->eop_out : mci->eop_in, mci->bounce_buffer, mci->blk_size, &tx);
